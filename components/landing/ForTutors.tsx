...
export default function ForTutors({
  ...
  const sectionFeaturedTutorsTitle = featuredTutorsTitle || t('landing.forTutors.featuredTutors');
  ...
  const getIconForBenefit = (benefitTitle: string) => {
    ...
    } else if (title.includes('cv') || title.includes('resume') || title.includes('build')) {
      return <GraduationCapIcon className="h-6 w-6" />;
    } else if (title.includes('hour') || title.includes('time') || title.includes('flex')) {
    ...
    } else if (title.includes('global') || title.includes('reach') || title.includes('world')) {
  ...
  const featuredTutors = [
    {
      ...
      students: 120,
      rating: 4.9,
      testimonial: 'Teaching on 11tutors has been an amazing experience. I\'ve earned enough to cover my textbooks and living expenses while helping others succeed.'
    },
    {
      name: 'David Kim',
      ...
      students: 145,
      rating: 4.8,
      testimonial: 'Creating courses has helped me master the material even better. Plus, the extra income as a student has been life-changing.'
    },
    {
      name: 'Sophia Chen',
      ...
      students: 180,
      rating: 4.9,
      testimonial: 'The platform makes it easy to create professional courses. I\'ve built a reputation as a tutor that\'s already helping my career prospects.'
    }
  ];
          ...
          >
            ...
            <motion.div variants={itemVariants} className="mb-16">
              <Button size="lg" className="group">
                {sectionJoinCtaText} <ArrowRightIcon className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </Button>
            </motion.div>
                    ...
                    <Card className="relative overflow-hidden border border-border/50 shadow-md hover:shadow-lg transition-all duration-300">
                        ...
                        <div className="flex gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1 text-sm text-muted-foreground">
                              <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] text-white font-bold ${tutor.universityColor}`}>
                      ...
                      <CardFooter className="bg-muted/20 px-6 py-3 flex justify-end">
                        <Button variant="ghost" size="sm" className="text-xs group">
                          View Profile <ArrowRightIcon className="ml-1 h-3 w-3 group-hover:translate-x-1 transition-transform" />
                        </Button>
                      </CardFooter>
                    </Card>
                  </motion.div>
